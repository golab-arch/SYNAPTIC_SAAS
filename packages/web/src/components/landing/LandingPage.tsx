    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-4xl mx-auto px-6 pt-20 pb-16 text-center">
        ...
        <div className="flex flex-col items-center gap-3 max-w-sm mx-auto">
          ...
          {/* Dev mode toggle */}
          <button onClick={() => setShowDev(!showDev)} className="text-xs text-gray-600 hover:text-gray-400 mt-2">
            {showDev ? 'Hide' : 'Dev mode'}
          </button>
          {showDev && (
            <div className="flex gap-2 w-full">
              <input value={devName} onChange={(e) => setDevName(e.target.value)} placeholder="Your name"
                className="flex-1 bg-gray-800 text-white rounded p-2 text-sm border border-gray-600 placeholder-gray-500" />
              <button onClick={() => auth.loginDev(devName)} className="px-4 py-2 bg-gray-700 text-white rounded text-sm hover:bg-gray-600">
                Enter
              </button>
            </div>
          )}
